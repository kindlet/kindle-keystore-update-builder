...
for model in ${KINDLE_MODELS} ; do
	...
	python3 kindle_update_tool.py m --${model} --sign ${ARCH}_install install.sh developer.keystore mergekeystore.jar

	# Build uninstall update
	python3 kindle_update_tool.py m --${model} --sign ${ARCH}_uninstall uninstall.sh
done
